...
function App() {
  ...
  useEffect(() => {
    // Bileşen ilk yüklendiğinde çalışıcak bir effect tanımlandı.İçinde API'ya istek atılır.
  ...
  const handleComplete = (id) => {
    // Tamamlandı butonuna tıklanırsa todo'nun unique id'si ile, tamamlandı işareti yapılır.
    fetch(`http://localhost:5000/todos/${id}/complete`, {
      // Backend'e PATCH isteği gönderilir.
      method: "PATCH", // HTTP isteğinin türü PATCH olarak ayarlanır.
    })
      .then((res) => res.json()) // Gelen cevabı JSON formatına çevirir.
      .then((updated) => {
        // Güncellenmiş todo objesi burada update olarak alınır.
        setTodos((prev) =>
  ...
  return (
    <div>
      ...
      <ul>
        ...
        {todos.map(
          (
            todo // todos dizisinde ki her bir todo için döngü başlatılır.
          ) => (
            <li key={todo._id}>
              {" "}
